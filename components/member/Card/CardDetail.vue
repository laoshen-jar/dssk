...
<div class="container">
    ...
    <div class="int" v-if="cardInfo.IsWriteOff==0">
        ...
        <span class="text" style="color:grey">{{cardInfo.StartDate|DateFilter}}<span class="day" v-if="cardState=='未开始'">未开始</span></span>
    </div>
    <div class="division" v-if="cardInfo.IsWriteOff==0"></div>
    <!-- int -->
    <div class="int" v-if="cardInfo.IsWriteOff==0">
        <span class="text" style="font-weight: 600">失效时间：</span>
        <span class="text" style="color:grey">{{cardInfo.EndDate|DateFilter}}<span class="day" v-if="cardState=='过期'">已过期</span></span>
    </div>
    ...
    <div class="store" v-if="cardInfo.ApplyStores&&cardInfo.ApplyStores.length>0">
        ...
        <ul class="store-list clearfix" style="color:grey">
            <li v-for="(item,index) in cardInfo.ApplyStores" :key="index" v-show="item.ApplyDishTypeItems.length>0">{{item.StoreName}}</li>
        </ul>
    </div>
...
<script>
import {GetCardInfo,GetAvailablestores} from "@/api/card"
...
export default {
    ...
    created() {
        Indicator.open('加载中...')
        var parameter = {}
        parameter.MemberCardCode = this.$route.query.MemberCardCode
        parameter.CardCode = this.$route.query.CardCode
        GetCardInfo(parameter).then(res => {
          ...
          if (res.state == 200) {
            ...
            else if(moment()<moment(res.MemberCard.StartDate,'YYYY-MM-DD HH:mm:ss')){this.cardState='未开始'}
            else{this.cardState='可用'}
