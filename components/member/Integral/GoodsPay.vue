	...
	<div class="container" v-if="loadding==0&&GoodsInfo">
		<div class="msg">
            ...
            <div class="img" :style="'background-image: url(' + GoodsInfo.Picture + ');'" />
              ...
              <span v-if="1-GoodsInfo.IntegralMaxRatio>0">￥{{ GoodsInfo.Price *parseInt((1-GoodsInfo.IntegralMaxRatio)*100)/100}}</span>
              <span v-if="GoodsInfo.IntegralMaxRatio<1&&GoodsInfo.IntegralMaxRatio>0"> + </span>
              <span v-if="GoodsInfo.IntegralMaxRatio>0">{{ GoodsInfo.Price *parseInt(GoodsInfo.IntegralMaxRatio* IntegralExchange*100)/100 }}积分</span>
            </p>
		...
		<div class="price">
			...
			<p class="cot" v-if="Member.IntegralBalance<IntegralNum"><span>{{ Member.IntegralBalance }}</span><span class="num" >（积分不足）</span></p>
		...
		<div class="btn-box">
			<div class="btn" v-if="!expire&&!Paying&&Member.IntegralBalance>=IntegralNum" @click="SubmitGoodsOrder">确认并支付</div>
  ...
  import { CreatePayOrder } from '@/api/pay'
	...
	export default {
		...
		computed: {
			...
			IntegralNum(){
				return (this.Member.IntegralBalance>this.GoodsInfo.MaxExchange?this.GoodsInfo.MaxExchange:this.Member.IntegralBalance)*this.PayCount
			},
			WxPayAmount(){
      ...
      this.NeedBusinessConfig().then(res2 => {
        ...
        this.IntegralExchange = this.$store.getters.BusinessConfig.IntegralExchange?this.$store.getters.BusinessConfig.IntegralExchange:10;
    			...
    			this.loadding = this.loadding -1;
					...
					}).finally(()=>{this.loadding = this.loadding -1;})
				});
			});
		...
		methods: {
			//购买数量加减事件
			PayCountReduce(){if(this.PayCount>1){this.PayCount--}},
			PayCountPlus(){this.PayCount++},
    ...mapActions(["NeedMember", "NeedBusinessConfig"]),
      ...
      SubmitGoodsOrder(){
          ...
          PayWechat:(parseInt(this.GoodsInfo.Price*this.PayCount*100)/100-this.PayIntegral).toFixed(2),
...
<style lang="scss" scoped>
    ...
    .btn_disable {
		...
		.editnumberbox{max-width:120px;float:right;margin-top:8px}
		.editnumberbox .available{background:#ffbb05}
		.editnumberbox .disable{background:#C0C4CC}
		.editnumberbox .btn{display: inline-block;position: relative;width: 25px;border-radius: 50%;}
		.editnumberbox .btn i{margin-left:5px;color: #fff;line-height: 25px;}
		.editnumberbox .number{display: inline-block; min-width: 30px; text-align: center; line-height: 26px; vertical-align: center;}
</style>