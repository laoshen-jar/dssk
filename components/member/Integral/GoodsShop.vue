<template>
  <div class="container" >
    ...
    <div class="can-use">
      ...
      <router-link to="/member/Integral/IntegralDetail" class="link">
        <p class="text">积分详情</p>
    ...
    <!-- shop-list -->
    <div class="shop-list-box" ref="wrapper" style="position: absolute;left: 0;top: 50px;right: 0;bottom: 0;overflow-y: scroll;">
      <mt-loadmore
        :bottom-method="LoadNextPage"
      ...
      >
        <div class="shop-list">
          <ul class="list clearfix">
            <li v-for="(item, index) in GoodsList" :key="index">
              <router-link
                ...
                :to="{
                  name: 'GoodsInfo',
                  query: { GoodsCode: item.GoodsCode }
                }"
              >
                ...
                <div class="text-box">
                  ...
                  <p style="text-align:-webkit-center">
                    ...
                    <span class="Price" v-if="1-item.IntegralMaxRatio>0">￥{{ item.Price *parseInt((1-item.IntegralMaxRatio)*100)/100}}</span><span class="Price" v-if="item.IntegralMaxRatio<1&&item.IntegralMaxRatio>0"> + </span><span class="Price" v-if="item.IntegralMaxRatio>0">{{ item.Price *parseInt(item.IntegralMaxRatio* IntegralExchange*100)/100 }}积分</span>
          ...
          <p v-if="allLoaded == false" style="font-size:12px;color:#777777;text-align:center"  @click="LoadNextPage">
            点击加载更多
          </p>
...
import { GetGoodsList } from "@/api/GoodsShop";
  ...
  created() {
    ...
    this.NeedMember().then(res => {
      ...
      this.NeedBusinessConfig().then(res2 => {
        console.log('this.NeedBusinessConfig().finally(res2',res2,this.$store.getters.BusinessConfig);
        this.IntegralExchange = this.$store.getters.BusinessConfig.IntegralExchange;
      })
      .finally(res => {this.loadding = this.loadding - 1;});
      // 请求数据
      this.LoadData();
    })
    .finally(res => {this.loadding = this.loadding - 1;});
