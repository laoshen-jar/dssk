<template>
  <div class="container">
    ...
    <div class="member-card-box" v-if="Member">
      ...
      v-if="BusinessConfig.MemberConfig&&BusinessConfig.MemberConfig.MemberCardPicture" 
      :style="'background: url('+BusinessConfig.MemberConfig.MemberCardPicture+') no-repeat;background-size: 100%;'">
        <p class="member-id" v-cloak 
        :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">
        {{ Member.NickName }}</br>
        NO. {{ Member.MemberNumber }}</p>
        <div class="data-box" hidden>
          <ul class="data-list clearfix">
            <li>
              <router-link to="/member/Integral/IntegralDetail">
                <p class="num" v-cloak :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">{{ Member.IntegralBalance }}</p>
                <p class="text" :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">积分</p>
              </router-link>
            </li>
            <li>
              <router-link to="/member/stored-detail">
                <p class="num" v-cloak :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">￥{{ Member.StoredBalance }}</p>
                <p class="text" :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">储值</p>
              </router-link>
            </li>
            <li>
              <router-link to="/member/card-list">
                <p class="num" v-cloak :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">{{ Member.MemberCardCount }}张</p>
                <p class="text" :style="'color:'+BusinessConfig.MemberConfig.MemberCardFontColor">优惠券</p>
              </router-link>
            </li>
          </ul>
        </div>
    ...
    <div class="cellbox">
      <div class="cellline" hidden>
        <div class="head" :style="'background: url('+Member.HeadImgUrl+') no-repeat;background-size: 80%;' "></div>
        ...
        <ul class="list" v-if="billList&&billList.length>0">
          <li v-for="(item, index) in billList" :key="index" @click="handleDetail(item)">
            <div class="item">
              ...
              <div class="state commented" v-if="moment(item.BillCashierTime).add(2,'days')<moment()">完成</div>
              <div class="state eating" v-else-if="item.BillState == 1">就餐中</div>
              <div class="state commented" v-else-if="item.CommentState == 1">已评价</div>
              <div class="state gocomment" v-else-if="item.BillState == 2 && item.CommentState == 0">去评价</div>
            </div>
...
<script>
import {  GetMemberRankList,  GetMemberRankDetail,  GetPrivilegeList,  GetEmployee} from "@/api/member";
...
export default {
  ...
  computed: {
    ...mapGetters(["MemberCode", "StoreCode", "DeskID", "Member",'Business',"BusinessConfig",'MediaService','Branch','OnActivity']),
    ShowTags(){
  ...
  created() {
    ...
    this.NeedMember().then(res => {
      if(!this.OnActivity){
        GetBillList({ MemberCode: this.MemberCode, pageNo: this.pageNo, pageSize: 3 }).then(res => {
  ...
  methods: {
      ...mapActions(["NeedMember", "NeedBusinessConfig"]),
        ToManage(){
            console.log('Business',this.Business,'http://'+this.Business.DomainName+'/user/login?state='+this.Employee.EmployeeCode+'dttd'+moment(new Date()).valueOf());
            window.location.href='http://'+this.Business.DomainName+'/user/login?state='+this.Employee.EmployeeCode+'dttd'+moment(new Date()).valueOf();
        },
        handleDetail(item) {
          if (item.BillState == 2 && item.CommentState == 0&&moment(item.BillCashierTime).add(2,'days')>moment()) {
              this.$router.push({
              name: "comment",
              query: {
                  BillID: item.BillID,
...
.cellbox{padding: 0 20px;}
.cellline{background: #fff;border-radius: 5px;-webkit-box-shadow: 0px 0px 3px 0px rgba(0,0,0,0.1);box-shadow: 0px 0px 3px 0px rgba(0,0,0,0.1);margin: 10px auto;padding: 0 15px 0 20px;position: relative;}
.cellline .title{ line-height: 50px;height: 50px;}
.cellline .head{height: 80px;line-height: 80px;border-radius: 50%;width: 80px;;}
.cellline .info { position:absolute;right:25px;height: 50px;line-height: 50px;top: 0;color:#ffbb05 ;}
.cellline .iconfont{position:absolute;right:10px;top:0px;height: 50px;line-height: 50px;color:  rgba(0,0,0,0.3);font-size: 14px;}
.fontyellow{color:#ffbb05 ;}
...
.manageBtnBox{padding:15px 15px 0;background: #f2f2f6;}
.manageBtn{width: 100%;height: 44px;line-height: 44px; border-radius: 20px; text-align:  center;color: #fff;  font-size: 14px;
background: #ff423e;box-shadow: 0px 0px 9.9px 0.1px rgba(255, 66, 62, 0.5);}
...
/* diningrecord end */
    ...
    .bannerimgitem{width: 100%;height: 100%;background-repeat:no-repeat;background-size: cover;background-position: center}
