<template>
	<div class="container">
			...
			<load-refresh :currentPage="usableCards.Cards.length" :totalPages="count" @refresh="loadBottom"
				...
				<div slot="content-list">
					<div class="notuse" v-if="usableCards.active">
							<ul class="list">
								<li v-for="(card,index) in usableCards.Cards" :key="index">
									<navigator :url="'/pages/cardDetail/index?MemberCardCode=' + card.MemberCardCode + '&CardCode=' + card.CardCode" class="item">
										<div class="msg">
											...
											<div class="text-box">
												...
												<p class="condition">使用条件：<span
														v-if="card.CardType!='礼品券'">消费满{{card.SatisfyAmount}}元可用</span><span
														v-else>消费满{{card.SatisfyAmount}}元可用,门店核销线下核销</span></p>
												<p class="date">有效期：{{card.EndDate|DateFilter}}</p>
												<!-- <p class="tip">{{card.expire}}天到期</p> -->
											</div>
										</div>
										<div class="select-box">
											<p :class="moment(card.StartDate)>moment()?'red':'text'">
												{{moment(card.StartDate)>moment()?'未开始':'使 用'}}
											</p>
										</div>
									</navigator>
								</li>
							...
							<!-- coupon-list end -->
							<uni-transition mode-class="fade" :show="usableCards.noresult">
								<div class="noresult" v-if="usableCards.noresult">
									<div class="noresult-cot">
										<div class="img">
											<img :src="staticUrl+ '/images/noresult.png'" alt="无结果" />
										</div>
						...
						<div class="used" v-if="usedCards.active">
							...
							<!-- coupon-list end -->
							<uni-transition mode-class="fade" :show="usedCards.noresult">
						...
						<div class="overtime" v-if="expiredCards.active">
							<ul class="list">
								<li v-for="(card,index) in expiredCards.Cards" :key="index">
									<navigator :url="'/pages/cardDetail/index?MemberCardCode=' + card.MemberCardCode + '&CardCode=' + card.CardCode" class="item">
										class="item">
										<div class="msg">
											<div class="text-box">
							...
							<!-- coupon-list end -->
							<uni-transition mode-class="fade" :show="expiredCards.noresult">
		...
		<!-- coupon-list end -->
		<uni-transition mode-class="fade" :show="noresult">
		...
		onLoad() {
			...
			GetCardList(parameter).then(res => {
				if (res.state == 200) {
					...
					this.usableCards.Cards = res.data.filter(item => moment(moment(item.EndDate).format(
						"YYYY-MM-DD 23:59:59")) > moment() && item.IsWriteOff == 0)

					this.usedCards.Cards = res.data.filter(item => item.IsWriteOff == 1)

					this.expiredCards.Cards = res.data.filter(item => item.IsWriteOff == 0 && moment(moment(item
						.EndDate).format("YYYY-MM-DD 23:59:59")) < moment())
