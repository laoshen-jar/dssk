<template>
	<div class="container">
		...
		<div class="score-record" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
			...
			<load-refresh :currentPage="IntegralRecord.length" :totalPages="count" @refresh="loadBottom"
				ref="loadRefresh" v-if="IntegralRecord.length>0">
				<div slot="content-list">
					<ul class="record-list">
						<li v-for="(item,index) in IntegralRecord" class="record-list-item" :key="index">
							<div class="text-box">
								<h6 class="title">{{item.SceneID}}</h6>
								<p class="time">{{dateFormart(item.CreateTime,'YYYY-MM-DD HH:mm:ss')}}</p>
							</div>
							<div class="data-box">
								<p :class="item.AddIntegral>0?'change':'give'">
									{{item.AddIntegral>0?"+":""}}{{item.AddIntegral}}
								</p>
								<p class="state">{{item.state}}</p>
							</div>
						</li>
					</ul>
					<p v-if="allLoaded==false"
						style="font-size:12px;color:#777777;text-align:center;margin-top:12px;line-height:30px"
		...
		onLoad() {
			...
			GetIntegralRecord(params).then(res => {
				...
				if (res.state == 200) {
					...
					this.$showToast("会员积分记录加载失败！")
				}
			}).catch(err => {
				console.log('err', err);
				this.$showToast("会员积分记录加载失败！" + err)
			})
		},
		methods: {
