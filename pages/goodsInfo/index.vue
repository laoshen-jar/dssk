<template>
	<div class="container" v-if="loadding==0">
		...
		<div class="box">
			<div class="manay">
				<span class="spend">
					<span v-if="1-GoodsInfo.IntegralMaxRatio>0"><span
							style="font-size:12px">￥</span>{{ GoodsInfo.Price * parseInt((1-GoodsInfo.IntegralMaxRatio)*100)/100}}</span>
					<span v-if="GoodsInfo.IntegralMaxRatio<1&&GoodsInfo.IntegralMaxRatio>0"> + </span>
					<span
						v-if="GoodsInfo.IntegralMaxRatio>0">{{ parseInt(GoodsInfo.Price *GoodsInfo.IntegralMaxRatio* IntegralExchange*100)/100 }}积分</span>
				</span>
			...
			<div class="data">
				...
				<span class="num">
					库存
					{{(GoodsInfo.EffectiveStock > GoodsInfo.Stock? GoodsInfo.Stock: GoodsInfo.EffectiveStock)>500?'充足':(GoodsInfo.EffectiveStock > GoodsInfo.Stock? GoodsInfo.Stock: GoodsInfo.EffectiveStock)}}
				</span>
			</div>
		</div>
		...
		<div class="box">
			...
			<div class="text">
				<div v-for="(card, index) in GoodsInfo.GoodsCards" :key="index" class="cardlist">
				...
				<div class="notuse" v-if="false">
					<ul class="list">
						<li v-for="(card, index) in GoodsInfo.GoodsCards" :key="index">
							<div class="item">
								<div class="msg">
									<div class="text-box">
										<h6 class="cardtitle">{{ card.CardName }}</h6>
		...
		<!-- int end -->
		<!-- detail -->
		<div class="detail" v-if="GoodsInfo.PictureMove && GoodsInfo.PictureMove.length > 0">
			<div class="img" v-for="(item, index) in GoodsInfo.PictureMove" :key="index">
				<img :src="item" />
			</div>
			<!-- <p class="text">{{GoodsInfo.Content}}</p> -->
		</div>
		...
		<div class="btn-box">
			...
			<div class="btn_disable"
				v-else-if="GoodsInfo.LimitNumber==GoodsInfo.OrderCount && GoodsInfo.LimitNumber != 0">已达到限兑数量</div>
			<navigator v-else class="btn" :url="'/pages/goodsPay/index?GoodsCode=' + GoodsInfo.GoodsCode">立即购买
			</navigator>
